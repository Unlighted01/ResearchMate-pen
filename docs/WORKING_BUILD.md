...
```cpp
// Driver: Panel_ILI9341 on SPI2_HOST (FSPI)
// DO NOT change to Panel_ST7789 or SPI3_HOST — tested and broken.
...
```
TFT Display:  MOSI=35, CLK=37, MISO=36, CS=38, DC=14, RST=21, BL=47
...
5. **`tft.init()` must run early** in `setup()` — do not add long delays before it. The ILI9341 has strict power-on timing requirements.
